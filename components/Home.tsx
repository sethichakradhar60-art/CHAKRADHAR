...
import { Play, Gem, Star, Github } from 'lucide-react';
...
export const Home: React.FC = () => {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-800 via-slate-900 to-black text-white p-6 relative overflow-hidden">
      ...
      {/* Background Decor */}
      <div className="absolute top-10 left-10 text-rose-500/20 animate-pulse-slow pointer-events-none">
        <Gem size={120} />
      </div>
      <div className="absolute bottom-20 right-10 text-blue-500/20 animate-bounce-short pointer-events-none">
        <Gem size={80} />
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center gap-8 max-w-md w-full">
        ...
        >
          ...
          <button className="relative w-full bg-slate-900 rounded-xl py-6 px-8 flex items-center justify-center gap-4 hover:bg-slate-800 transition-all border border-slate-700 text-white">
        ...
        <div className="grid grid-cols-2 gap-4 w-full text-center text-sm text-slate-500">
          ...
          <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700/50">
            <p className="font-bold text-white mb-1">High Score</p>
            <p>Tracking</p>
          </div>
        </div>
        
        <div className="mt-8 text-slate-600 flex items-center gap-2 text-xs">
          <Github size={14} />
